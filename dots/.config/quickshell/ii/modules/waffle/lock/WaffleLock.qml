...
LockScreen {
    ...
    component ClockTextGroup: Column {
        ...
        WText {
            ...
            text: {
                const formatted = DateTime.formatTime();
                const match = formatted.match(/(\d{1,2})(\D+)(\d{2})(?:\D+(\d{2}))?/);
                return match ? `${match[1]}${match[2]}${match[3]}${match[4] ? `${match[2]}${match[4]}` : ""}` : formatted;
            }
        }
