...
Canvas { // Visualizer
    ...
    property list<var> shapeContour: []
    property point contourCenter: Qt.point(0, 0)
    ...
    onRoundedPolygonChanged: updateShapeCache()
    onWidthChanged: updateShapeCache()
    onHeightChanged: updateShapeCache()
    onContourSamplesPerCubicChanged: updateShapeCache()
    ...
    function updateShapeCache() {
        ...
        shapeContour = getShapeContour(size, xOffset, yOffset);

        var mappedCenter = root.roundedPolygon?.center
            ? root.mapPolygonPoint(root.roundedPolygon.center, size, xOffset, yOffset)
            : null;
        contourCenter = mappedCenter ? Qt.point(mappedCenter.x, mappedCenter.y) : Qt.point(w / 2, h / 2);
        root.requestPaint();
    }
    ...
    onPaint: {
        ...
        var contour = root.shapeContour;
        var cx = root.contourCenter.x;
        var cy = root.contourCenter.y;
    ...
    Component.onCompleted: updateShapeCache()
}
